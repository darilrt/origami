...
int main(int argc, char **argv)
{
  ...
  if (WGLEW_ARB_extensions_string || WGLEW_EXT_extensions_string)
  {
    ...
    PrintExtensions(wglGetExtensionsStringARB ? (const char *)wglGetExtensionsStringARB(ctx.dc) : (const char *)wglGetExtensionsStringEXT());
  }
#elif defined(__APPLE__) && !defined(GLEW_APPLE_GLX)
...
void VisualInfoARB(GLContext *ctx)
{
  ...
  if (!verbose)
  {
    ...
    for (i = 1; i <= maxpf; i++)
    {
      ...
      if (!showall && ((value[2] && !value[1]) || (!WGLEW_ARB_pbuffer || !value[n_pbuffer]) || (value[3] != WGL_FULL_ACCELERATION_ARB)))
        continue;
      ...
      /* acceleration */
      fprintf(file, "%s ", value[3] == WGL_FULL_ACCELERATION_ARB ? "fu" : value[3] == WGL_GENERIC_ACCELERATION_ARB ? "ge"
...
void VisualInfoGDI(GLContext *ctx)
{
  ...
  if (!verbose)
  {
    ...
    for (i = 1; i <= maxpf; i++)
    {
      DescribePixelFormat(ctx->dc, i, sizeof(PIXELFORMATDESCRIPTOR), &pfd);
      /* only describe this format if it supports OpenGL */
      if (!(pfd.dwFlags & PFD_SUPPORT_OPENGL) || (drawableonly && (pfd.dwFlags & PFD_DRAW_TO_BITMAP)))
        continue;
  ...
  else /* verbose */
  {
    ...
    for (i = 1; i <= maxpf; i++)
    {
      DescribePixelFormat(ctx->dc, i, sizeof(PIXELFORMATDESCRIPTOR), &pfd);
      /* only describe this format if it supports OpenGL */
      if (!(pfd.dwFlags & PFD_SUPPORT_OPENGL) || (drawableonly && !(pfd.dwFlags & PFD_DRAW_TO_WINDOW)))
        continue;
      fprintf(file, "Visual ID: %2d  depth=%d  class=%s\n", i, pfd.cDepthBits,
              pfd.cColorBits <= 8 ? "PseudoColor" : "TrueColor");
...
GLboolean CreateContext(GLContext *ctx)
{
  ...
  if (FALSE == SetPixelFormat(ctx->dc, visual, &pfd))
    return GL_TRUE;
  /* create rendering context */
  ctx->rc = wglCreateContext(ctx->dc);
  if (NULL == ctx->rc)
    return GL_TRUE;
  if (FALSE == wglMakeCurrent(ctx->dc, ctx->rc))
    return GL_TRUE;
  return GL_FALSE;
}
...
GLboolean CreateContext(GLContext *ctx)
{
  ...
  if (NULL == ctx->dpy)
    return GL_TRUE;
  /* query for glx */
  if (!glXQueryExtension(ctx->dpy, &erb, &evb))
    return GL_TRUE;
  /* choose visual */
  ...
  if (!glXMakeCurrent(ctx->dpy, ctx->wnd, ctx->ctx))
    return GL_TRUE;
  return GL_FALSE;
}
...
void DestroyContext(GLContext *ctx)
{
  if (NULL != ctx->dpy && NULL != ctx->ctx)
    glXDestroyContext(ctx->dpy, ctx->ctx);
  if (NULL != ctx->dpy && 0 != ctx->wnd)
    XDestroyWindow(ctx->dpy, ctx->wnd);
  if (NULL != ctx->dpy && 0 != ctx->cmap)
